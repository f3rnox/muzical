...
export default function AlbumList(props: Readonly<AlbumListProps>) {
	...
	const selectedIndex = albumsForSelectedArtist
		.findIndex(({ name }): boolean => name === selectedAlbum?.name)
	...
	const title = totalCount === albumsForSelectedArtist.length
	...
	return (
		...
		>
			...
			{visible.map((album: LibraryAlbum): React.ReactElement => {
				...
				return (
					<Text key={`${name}-${year ?? ''}`} wrap="truncate-end" {...textProps}>
						{isSelected ? '▸ ' : '  '}
