...
import { spectrumRowColor, type SpectrumRowColor } from '../utils/spectrum_row_color'
...
export default function VisualizerView(props: Readonly<VisualizerViewProps>) {
	...
	const rowChars: string[] = useMemo((): string[] => {
		...
		return Array.from({ length: totalRows }, (_: unknown, r: number): string => {
			const rowFromBottom = totalRows - 1 - r
			return heightEighths
				.map((he: number): string => `${spectrumCellChar(rowFromBottom, he)} `)
				.join('')
		})
	}, [heights, totalRows])

	const title: string = playingSong?.metadata.common.title
		?? (playerName === null ? 'No player detected' : 'Idle')
	const artist: string = playingSong?.metadata.common.artist ?? ''
	const stateLabel: string = playingSong === null
		? (playerName === null ? '⚠ no player detected' : `■ idle · ${playerName}`)
		: `▶ playing · ${playerName ?? 'unknown'}`
	const stateColor: 'green' | 'gray' | 'red' = playingSong === null
		...
		>
			...
			<Box flexDirection="column">
				{rowChars.map((line: string, rowIndex: number): React.ReactElement => {
					...
					const color: SpectrumRowColor = spectrumRowColor(rowFromBottom, totalRows)
					return (
						<Text key={`row-${rowFromBottom}`} color={color}>{line}</Text>
					)
				})}
			</Box>
			<Box marginTop={1}>
