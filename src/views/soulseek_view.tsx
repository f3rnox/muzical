 ...
 */
function ResultRow(props: Readonly<{
	...
	const bitrateLabel = result.bitrate !== null ? `${result.bitrate}kbps` : '----'
	const durationLabel = result.durationSec !== null
		? formatDuration(result.durationSec)
		: '--:--'
	const sizeLabel = formatBytes(result.size)
	const userLabel = truncate(result.username, 14).padEnd(14, ' ')
	const basename = result.filename.replaceAll('\\', '/').split('/').pop() ?? result.filename
...
function DownloadRow(props: Readonly<{
	...
	const detail = download.error !== null
		? download.error
...
export default function SoulseekView(props: Readonly<SoulseekViewProps>) {
	const { musicDir, soulseekUsername, soulseekPassword, maxRows, setAppView, exit } = props
	const {
		status,
		statusMessage,
		results,
		...
		download,
		clearResults
	} = useSoulseek({ musicDir, username: soulseekUsername, password: soulseekPassword })
	...
	const { start, end } = useScrollWindow(results.length, clampedSelection, resultRows)
	const visible = useMemo((): SoulseekSearchResult[] => (
	...
	useInput((input: string, key: Key): void => {
		...
		if (key.pageUp) {
			setSelectedIndex((prev: number): number => Math.max(0, prev - resultRows))
	...
	const resultsTitle = results.length > 0
		? `Results ${results.length}${isSearching ? ' (searching…)' : ''}`
		: isSearching ? 'Results (searching…)' : 'Results'
	...
	return (
		...
		>
			...
			>
				...
				{visible.map((result: SoulseekSearchResult, index: number): React.ReactElement => {
					const absoluteIndex = start + index
					return (
						<ResultRow
							key={result.id}
							result={result}
							isSelected={absoluteIndex === clampedSelection && mode === 'list'}
						/>
					)
				})}
				{end < results.length ? (
			...
			>
				{visibleDownloads.length === 0 ? (
					...
					visibleDownloads.map((entry: SoulseekDownload): React.ReactElement => (
