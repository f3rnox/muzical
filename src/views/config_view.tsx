...
function parseExtensions(raw: string): string[] {
	return raw
		...
		.map((entry: string): string => entry.startsWith('.') ? entry : `.${entry}`)
}
 ...
 */
function applyField(config: Readonly<Config>, id: FieldId, draft: string): Config {
	if (id === 'musicDir') {
...
export default function ConfigView(props: Readonly<ConfigViewProps>) {
	...
	useInput((input: string, key: Key): void => {
		...
		if (key.downArrow || input === 'j') {
			setFocusIndex((prev: number): number => Math.min(FIELDS.length - 1, prev + 1))
	...
	return (
		...
		>
				...
				return (
					...
					>
						<Text>
							...
							<Text color={labelColor} bold={isFocused}>{field.label}</Text>
			...
			<Box marginTop={1} flexDirection="column">
				...
				<Text dimColor>changes apply to the config file instantly; library re-scan requires restart</Text>
			</Box>
		</TitledBox>
	)
}
