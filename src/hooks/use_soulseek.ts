...
export type SoulseekDownloadStatus =
	| Download['status']
	| 'saving'
	| 'failed'
 ...
 */
export function useSoulseek(options: Readonly<UseSoulseekOptions>): UseSoulseekApi {
	const { musicDir, username, password } = options
	const clientRef = useRef<SlskClient | null>(null)
	const [status, setStatus] = useState<SoulseekConnectionStatus>('disconnected')
	...
	const patchDownload = useCallback((
		id: string,
		patch: Partial<SoulseekDownload>
	): void => {
		setDownloads((prev: SoulseekDownload[]): SoulseekDownload[] => (
			prev.map((entry: SoulseekDownload): SoulseekDownload => (
		...
		try {
			await client.search(trimmed, {
				onResult: (response): void => {
					...
					setResults((prev: SoulseekSearchResult[]): SoulseekSearchResult[] => (
						[...prev, ...additions]
					))
				}
			})
	...
	const download = useCallback(async (result: SoulseekSearchResult): Promise<void> => {
		const client = clientRef.current
		...
		const destinationPath = buildDownloadPath(musicDir, result.username, result.filename)
		...
		requested.events.on('status', (
			nextStatus: Download['status']
		): void => {
			patchDownload(downloadId, { status: nextStatus })
		})

		requested.events.on('progress', (metadata: {
			totalBytes: bigint
			receivedBytes: bigint
			progress: number
		}): void => {
