...
export function useAppInput(handlers: Readonly<AppInputHandlers>): void {
	...
	const handleQuitAndViewSwitch = useCallback(
	...
	const handlePlaybackAndSearch = useCallback(
		(input: string, key: Key): boolean => {
			if (input === " ") {
	...
	const handleLibraryHorizontalNav = useCallback(
		(input: string, key: Key): boolean => {
			if (appView !== AppView.Library) {
				return false;
			}
	...
	const handleListMovement = useCallback(
		(input: string, key: Key): boolean => {
			if (input === "<") {
				playPlaylistRelative(-1);
	...
	const handleVolumeAndClear = useCallback(
