...
export function createCli(): Command {
	...
	program
		...
		.option('--no-clear-screen', 'do not clear the terminal before/after running')
		.action(async (options: RootOptions): Promise<void> => {
			await runTui({
				...
				...(options.extension !== undefined && { songExtensions: options.extension }),
	...
	program
		...
		.option('--json', 'emit machine-readable JSON')
		.action(async (kind: ListKind, options: ListCommandOptions): Promise<void> => {
