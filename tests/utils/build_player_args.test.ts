...
describe("buildPlayerArgs", (): void => {
	describe("mpv", (): void => {
		...
		it("appends --input-ipc-server only when mpvIpcPath is provided", (): void => {
			...
			expect(
				without.some((arg: string): boolean =>
					arg.startsWith("--input-ipc-server="),
				),
			).to.equal(false);
		});
	...
	describe("mpg123", (): void => {
		it("uses -f scale of 256 at volume 100", (): void => {
			const args = buildPlayerArgs(candidate("mpg123"), "/song.mp3", {
				volume: 100,
			});
		...
		it("scales 0..100 volume linearly into 0..256", (): void => {
			const args50 = buildPlayerArgs(candidate("mpg123"), "/song.mp3", {
				volume: 50,
			});
	...
	describe("ffplay", (): void => {
		it("uses -af volume filter with decimal scaled 0..1", (): void => {
			const args = buildPlayerArgs(candidate("ffplay"), "/song.mp3", {
				volume: 50,
			});
